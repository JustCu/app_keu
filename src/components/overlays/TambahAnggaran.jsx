      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-8 px-4 mt-4">
        ...
        <div className="mb-6">
          <div className="bg-gray-100 p-1 rounded-xl flex gap-1 relative">
            <div
              className={`absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] bg-white rounded-lg shadow-sm transition-transform duration-300 ease-in-out ${tipe === "pemasukan" ? "translate-x-full" : "translate-x-0"}`}
            ></div>

            <button
              onClick={() => setTipe("pengeluaran")}
              className={`relative z-10 flex-1 text-sm py-2 transition-colors ${tipe === "pengeluaran" ? "font-bold text-red-600" : "font-medium text-gray-500"}`}
            >
              Pengeluaran
            </button>
            <button
              onClick={() => setTipe("pemasukan")}
              className={`relative z-10 flex-1 text-sm py-2 transition-colors ${tipe === "pemasukan" ? "font-bold text-green-600" : "font-medium text-gray-500"}`}
            >
              Pemasukan
            </button>
          </div>
        ...
        <div className="flex flex-col gap-5">
          <div>
            ...
            <input
              ...
              className="w-full bg-gray-50 border border-gray-200 text-gray-900 font-medium rounded-xl p-4 outline-none focus:ring-2 focus:ring-blue-500 transition"
            />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4 items-center mb-3">
              <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center text-3xl border border-gray-200 shrink-0 shadow-inner">
                {ikon || "❓"}
              </div>
              <input
                type="text"
              ...
              <div className="grid grid-cols-6 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`h-10 text-xl rounded-lg flex items-center justify-center transition-all ${ikon === emoji ? "bg-blue-100 border-blue-300 border shadow-sm" : "hover:bg-gray-200 bg-white border border-gray-100"}`}
                  >
                    {emoji}
                  </button>
                ))}
